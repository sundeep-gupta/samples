...
# CONFIG VARIABLES
  ...
  my $query = "select owner from fowner.v_fowner\$file where filename = lower('emgc_'||'".$short_lrgsuite.".tsc') and product='EMGC' and branch_name='MAIN' and platform_name in('LINUX','LINUX.X64') and rownum=1";
...
sub get_label_list{
 ...
 my @label_list = `ade showlabels -series $series 2>/dev/null`;
...
sub getReport_html{
  ...
  ###################GET HTML FOR DATA################################################################
    ...
    if($new_diffs > $redflag_diff_number){
     ...
     $rh_common_lrgs->{$comm_lrg} = {
       'comm_lrg' => $comm_lrg,
       'lrgowner' => $lrgowner,
       'base_run_lrg_suc' => $base_run_lrg_suc,
 ...
 #    my $lrgowner = &getlrgowner($comm_lrg);
 #   $output_str_for_html .= "<TR><TD >$comm_lrg</TD><TD >$lrgowner</TD><TD BGCOLOR='#F0E68C'>$base_run_lrg_suc</TD><TD BGCOLOR='#F0E68C'>$base_run_lrg_dif</TD><TD  BGCOLOR='#F0E68C'>$base_dif_str</TD><TD BGCOLOR='#F0E68C'>$tot_base</TD><TD BGCOLOR='#00FFFF'>$current_run_lrg_suc</TD><TD BGCOLOR='#00FFFF'>$current_run_lrg_dif</TD><TD BGCOLOR='#00FFFF'>$tot_current</TD><TD BGCOLOR='#FFC0CB'>$new_diffs_html</TD><TD BGCOLOR='#FFC0CB'>$tot_dif_html</TD><TD BGCOLOR='#FFC0CB'>$new_dif_str</TD><TD BGCOLOR='#FFC0CB'>$new_dif_pid_str</TD><TD BGCOLOR='#C0C0C0'>$ex_dif_pid_str</TD><TD>$curr_dif_str</TD></TR>";
  ####################################################################################################
  ####################################################################################################
  }
  my $ra_common_lrgs = [values %$rh_common_lrgs];
  my $ra_sorted_common_lrgs = sort { $a->{'new_diffs'} <=> $b->{'new_diffs'}; } @$ra_common_lrgs;
  foreach my $lrg ( @$ra_sorted_common_lrgs) {
    use Data::Dumper;
    print Dumper($lrg);
    print $lrg->{'common_lrg'};
    $output_str_for_html .= "<TR><TD >". $lrg->{'comm_lrg'} 
        . "</TD><TD >" . $lrg->{'lrgowner'} 
        ."</TD><TD BGCOLOR='#F0E68C'>" . $lrg->{'base_run_lrg_suc'}
        . "</TD><TD BGCOLOR='#F0E68C'>". $lrg->{'base_run_lrg_dif'}
        . "</TD><TD  BGCOLOR='#F0E68C'>" . $lrg->{'base_dif_str'}
        . "</TD><TD BGCOLOR='#F0E68C'>" . $lrg->{'tot_base'}
        . "</TD><TD BGCOLOR='#00FFFF'>" . $lrg->{'current_run_lrg_suc'}
        . "</TD><TD BGCOLOR='#00FFFF'>" . $lrg->{'current_run_lrg_dif'}
        . "</TD><TD BGCOLOR='#00FFFF'>" . $lrg->{'tot_current'}
        . "</TD><TD BGCOLOR='#FFC0CB'>" .$lrg->{'new_diffs_html'}
        . "</TD><TD BGCOLOR='#FFC0CB'>" . $lrg->{'tot_dif_html'}
        . "</TD><TD BGCOLOR='#FFC0CB'>" . $lrg->{'new_dif_str'}
        . "</TD><TD BGCOLOR='#FFC0CB'>" . $lrg->{'new_dif_pid_str'}
        . "</TD><TD BGCOLOR='#C0C0C0'>" . $lrg->{'ex_dif_pid_str'}
        . "</TD><TD>" . $lrg->{'curr_dif_str'} ."</TD></TR>";
  }
  my $has = \%hash;
...
$output_file_content_html .= '<TR BGCOLOR="#FFFF00"><TD>LRG-Name</TD><TD>OWNER</TD><TD>SUC</TD><TD>DIF</TD><TD>BASE PID</TD><TD>TOT</TD><TD>SUC</TD><TD>DIF</TD><TD>TOT</TD><TD>EXTRA_DIF</TD><TD>TOTAL_DIF</TD><TD>EXTRA DIF FILES</TD><TD>PIDS FOR EXTRA DIFS</TD><TD>DIFF HISTORY FROM OTHER LABEL</TD><TD>CURR_PID</TD></TR>';
####################################################################################
